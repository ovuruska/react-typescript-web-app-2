...
describe("CapacityRemoteDataSourceImpl", () => {
  ...
  beforeAll(() => {
    ...
    capacityRemoteDataSource = container.get(CapacityRemoteDataSource) as CapacityRemoteDataSourceImpl;
  ...
  it("should fetch monthly capacity successfully", async () => {
    ...
    const result = await capacityRemoteDataSource.getMonthlyCapacity(monthlyCapacityRequest);
    expect(mockAxios.post).toHaveBeenCalled();
    expect(result).toEqual(mockResponse);
    expect(mockAxios.post).toHaveBeenCalledWith('/api/capacity/monthly', monthlyCapacityRequest, undefined);
