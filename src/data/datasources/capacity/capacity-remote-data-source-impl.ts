...
export class CapacityRemoteDataSourceImpl implements CapacityRemoteDataSource {
  ...
  async getMonthlyCapacity(body: MonthlyCapacityRequest): Promise<MonthlyCapacityResponse> {
    const response = await this.client.post<MonthlyCapacityRequest, MonthlyCapacityResponse>('/api/capacity/monthly', body);
