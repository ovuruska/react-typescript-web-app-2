...
const SurgeCalendar: React.FC<SurgeCalendarProps> = ({
  ...
  useEffect(() => {
    ...
    const params: GetMonthlyCapacityParams = {
      date: dateString, employees: employees, branches: branches, service: service
    }
    onChange(date)
    getMonthlyCapacityUseCase.call(params).then((response: MonthlyCapacityResponse) => {
  ...
  return (<div>
    <CustomCalendar mapDateToClassName={mapDateToClassName} date={date} onChange={handleChange}/>
  </div>);
}
