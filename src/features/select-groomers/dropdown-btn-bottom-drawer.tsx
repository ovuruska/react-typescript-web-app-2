...
export interface DropdownBtnProps {
  label: string;
  options: EmployeeEntity[];
  onSelect?: (options: EmployeeEntity[]) => void;
  initialState?: EmployeeEntity[];
}
...
const DropdownBtnBottomDrawer = ({
  ...
  const [selected, setSelected] = useState<EmployeeEntity[]>(initialState ?? []);

  const handleCheck = (option: EmployeeEntity) => {
    setSelected((prev) => {
